"use client"
import { PersonRounded, LoginRounded, AssignmentRounded} from '@mui/icons-material';
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <nav className="text-sm text-gray-500">Pages / Dashboard</nav>
              <h1 className="text-lg font-semibold text-gray-900">Dashboard</h1>
            </div>
            <div className="flex items-center space-x-4">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                <Input placeholder="Type here..." className="pl-10 w-64" />
              </div>
              <Button variant="outline" size="sm" className="text-pink-600 border-pink-600 bg-transparent">
                ONLINE BUILDER
              </Button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center mt-2">
                        ...
                        <span className="text-sm text-gray-500 ml-1">{card.changeText}</span>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>(+15%) increase in today sales</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-48 bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-4 mb-4">
                  <div className="h-full flex items-end justify-between text-white">
                    {["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map((month, i) => (
                      <div key={month} className="flex flex-col items-center">
                        <div
                          className="w-2 h-2 bg-white rounded-full mb-1"
                          style={{ marginBottom: `${Math.random() * 60 + 10}px` }}
                        />
                ...
                <div className="space-y-4">
                  {orders.map((order, index) => (
                    <div key={index} className="flex items-start space-x-3">
                      <div className={`w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center ${order.color}`}>
